...
export function ReadyCheckModal({ open, room, userId, isHost, participants }: ReadyCheckModalProps) {
  ...
  if (!readyCheck || !readyCheck.active) return null;
  ...
  const everyoneReady = unreadyParticipants.length === 0;

  useEffect(() => {
    if (everyoneReady && isHost && room.videoUrl) {
      play("start");
  ...
  }, [everyoneReady, isHost, room.id, room.videoUrl]);
