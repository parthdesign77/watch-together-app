...
export function Avatar({ user, size = "md", showStatus = false }: AvatarProps) {
  ...
  return (
    <span className="relative inline-flex shrink-0">
      <span
        className={`${sizes[size]} inline-flex items-center justify-center overflow-hidden rounded-lg border border-white/15 bg-elevated font-bold text-white`}
      ...
      {showStatus && participant?.isSpeaking ? (
        <span className="absolute -inset-1 rounded-lg border-2 border-cyan shadow-glow" aria-hidden />
      ) : null}
      ...
      {showStatus && participant?.isScreenSharing ? (
        <MonitorUp className="absolute -bottom-1 -left-1 h-4 w-4 rounded bg-cyan p-0.5 text-white" aria-label="Screen sharing" />
      ) : null}
    </span>
  );
}
